...
class DataObject {
  ...
  public:
    ...
    virtual ~DataObject() { DataObject::totalObjects--; }

    // 拷贝构造函数（深拷贝）
    DataObject(const DataObject &other)
        : id(other.id), name(other.name), path(other.path), size(other.size), createTime(other.createTime),
          metadata(other.metadata), accessCount(0) {
        DataObject::totalObjects++;
    }
    ...
    static void resetTotalObjects() { totalObjects = 0; }

  protected:
    void updateSize(double newSize) { size = newSize; }
