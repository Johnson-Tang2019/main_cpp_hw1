...
class DataExporter {
  ...
  public:
    ...
    template <typename T> friend bool exportToJSON(const DataObject &data, const std::string &filename);

    // 批量导出
    template <typename T> int batchExport(const std::vector<T> &dataList, const std::string &format);
